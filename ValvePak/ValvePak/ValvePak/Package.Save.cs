...
namespace SteamDatabase.ValvePak
{
	public partial class Package
	{
		...
		public void Write(Stream stream, uint? version = null)
		{
			...
			// 使用检测到的编码，默认为 UTF-8
			var encoding = this.TextEncoding ?? Encoding.UTF8;
			...
			{
				writer.Write(encoding.GetBytes(typeEntries.Key));
				writer.Write(NullByte);

				foreach (var directoryEntries in typeEntries.Value)
				{
					writer.Write(encoding.GetBytes(directoryEntries.Key));
					writer.Write(NullByte);
					...
					foreach (var entry in directoryEntries.Value)
					{
						...
						writer.Write(encoding.GetBytes(entry.FileName));
						writer.Write(NullByte);
