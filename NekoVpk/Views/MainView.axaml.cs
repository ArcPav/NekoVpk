...
public class SteamBBCodeHelper
{
    ...
    private static void ParseBBCode(string text, InlineCollection inlines)
    {
        ...
        var regex = new System.Text.RegularExpressions.Regex(@"\[(/?[a-zA-Z0-9*_]+)(?:=([^\]]+))?\]", System.Text.RegularExpressions.RegexOptions.Compiled);
        int lastPos = 0;
        bool noparse = false;
        ...
        foreach (System.Text.RegularExpressions.Match match in regex.Matches(text))
        {
            ...
            if (match.Index > lastPos)
            {
                ...
                if (System.Linq.Enumerable.Any(stack, s => s.Tag == "url"))
    ...
    private static void AddTextWithAutoLinks(InlineCollection inlines, string text)
    {
        ...
        var regex = new System.Text.RegularExpressions.Regex(@"(https?://[-A-Za-z0-9+&@#/%?=~_|!:,.;]*[-A-Za-z0-9+&@#/%=~_|])", System.Text.RegularExpressions.RegexOptions.Compiled);
        int lastPos = 0;
